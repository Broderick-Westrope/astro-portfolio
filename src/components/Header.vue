...
<template>
  <header id="header" :class="{ 'backdrop-blur-sm': scroll > 20 }"
    ...
    <div class="flex gap-x-6">
      <a v-for="link in socialLinks" :key="link.text" :aria-label="`${link.text}`" :class="link.icon"
        class="iconify hover:text-primary transition-colors duration-200 cursor-pointer size-6"
        :target="getLinkTarget(link.href)" :href="link.href" />

      <a class="hover:text-primary transition-colors duration-200 cursor-pointer iconify akar-icons--rss size-6"
        target="_blank" href="/rss.xml" aria-label="RSS" />
      <ThemeToggle />
    </div>
  </header>
  <nav id="nav-drawer"
    ...
    <span class="iconify akar-icons--arrow-left size-6" @click="toggleNavDrawer()" />
    <a v-for="link in navLinks" :key="link.text" :aria-label="`${link.text}`" :target="getLinkTarget(link.href)"
      class="hover:text-primary transition-colors duration-200 cursor-pointer" :href="link.href"
      @click="toggleNavDrawer()">
      {{ link.text }}
    </a>
  </nav>
  <div id="nav-drawer-mask"
